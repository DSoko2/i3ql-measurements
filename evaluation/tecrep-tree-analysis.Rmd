...
onlyPrioLoad <- function(data) {
  return(data %>% filter(costFunction %in% c("prio\U03A0", "prio\U03A3", "load\U03A0", "load\U03A3")))
}
```
...
plotPlacementStats <- function(data, titleAddition = "") {
  colors <- c("cyan", "yellow", "red")
  return(ggplot(data, aes(y=selectivity, x=query)) +
          ggtitle(paste("Optimal Placements", titleAddition)) +
          ...
          scale_fill_gradientn(colours=colors, limits=c(1, 8)) +
          theme(
              legend.position = "none",
             # legend.position = c(case_when(legendRight ~ 0.e91, TRUE ~ 0.1), 0.9),
...
placementStatsHomoPrivPlot <- plotPlacementStats(placementStatistics %>% filter(resources == "homogeneous" & placement == "private"), "(Privacy-Aware)")
# Export
ggsave("stability-homo-private.pdf", width = 10, height = 5.4, units = "cm", device = cairo_pdf)
placementStatsHeteroPrivPlot <- plotPlacementStats(placementStatistics %>% filter(resources == "heterogeneous" & placement == "private"), "(Heterogeneous Setup)")
# Export
ggsave("stability-hetero-private.pdf", width = 10, height = 5.4, units = "cm", device = cairo_pdf)

placementStatsHomoOptPlot <- plotPlacementStats(placementStatistics %>% filter(resources == "homogeneous" & placement == "optimal"), "")
# Export
ggsave("stability-homo-optimal.pdf", width = 10, height = 5.4, units = "cm", device = cairo_pdf)
placementStatsHeteroOptPlot <- plotPlacementStats(placementStatistics %>% filter(resources == "heterogeneous" & placement == "optimal"), "(Heterogeneous Setup)")
# Export
ggsave("stability-hetero-optimal.pdf", width = 10, height = 5.4, units = "cm", device = cairo_pdf)
