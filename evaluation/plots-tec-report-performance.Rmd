...
plotRt <- function(data, title, legendRight = TRUE, fillShift = 120) {
  return(ggplot(data, aes(y=relativeRuntime, x=query, fill=placement)) +
          ...
          scale_fill_hue(h.start=fillShift) +
          plotTheme() +
...
plotMug <- function(data, title, legendRight = TRUE, minY = 0.8, fillShift = 120) {
...
plotCpu <- function(data, title, legendRight = TRUE, fillShift = 120) {
...
homoPerfRtPlot <- plotRt(rtHomoPerf %>% filter(resources == "homogeneous"), expression(paste("Run Time (", S[h], ")")), TRUE, 240)
# Export
ggsave("selectivity-runtime-homo.pdf", width = 10, height = 5, units = "cm", device = cairo_pdf)
homoPerfRtPlotEstRel <- plotRt(rtHomoPerfEstRel %>% filter(resources == "homogeneous"), expression(paste("Run Time (", S[h], ")")), FALSE, 240)
# Export
ggsave("selectivity-runtime-homoEstRel.pdf", width = 10, height = 5, units = "cm", device = cairo_pdf)
heteroPerfRtPlot <- plotRt(rtHeteroPerf %>% filter(resources == "heterogeneous"), expression(paste("Run Time (", S[v], ")")), TRUE, 240)
# Export
ggsave("selectivity-runtime-hetero.pdf", width = 10, height = 5, units = "cm", device = cairo_pdf)
heteroPerfRtPlotEstRel <- plotRt(rtHeteroPerfEstRel %>% filter(resources == "heterogeneous"), expression(paste("Run Time (", S[v], ")")), TRUE, 240)
# Export
...
homoPerfCpuPlot <- plotCpu(cpuHomoPerf %>% filter(resources == "homogeneous"), expression(paste("Total CPU Time (", S[h], ")")), FALSE, 240)
# Export
ggsave("selectivity-cpu-homo.pdf", width = 10, height = 5, units = "cm", device = cairo_pdf)
homoPerfCpuPlotEstRel <- plotCpu(cpuHomoPerfEstRel %>% filter(resources == "homogeneous"), expression(paste("Total CPU Time (", S[h], ")")), TRUE, 240)
# Export
ggsave("selectivity-cpu-homoEstRel.pdf", width = 10, height = 5, units = "cm", device = cairo_pdf)
heteroPerfCpuPlot <- plotCpu(cpuHeteroPerf %>% filter(resources == "heterogeneous"), expression(paste("Total CPU Time (", S[v], ")")), FALSE, 240)
# Export
ggsave("selectivity-cpu-hetero.pdf", width = 10, height = 5, units = "cm", device = cairo_pdf)
heteroPerfCpuPlotEstRel <- plotCpu(cpuHeteroPerfEstRel %>% filter(resources == "heterogeneous"), expression(paste("Total CPU Time (", S[v], ")")), TRUE, 240)
# Export
...
homoPerfMugPlot <- plotMug(mugHomoPerf %>% filter(resources == "homogeneous"), expression(paste("Total Memory Growth (", S[h], ")")), FALSE, 0.7, 240)
# Export
ggsave("selectivity-memory-homo.pdf", width = 10, height = 5, units = "cm", device = cairo_pdf)
homoPerfMugPlotEstRel <- plotMug(mugHomoPerfEstRel %>% filter(resources == "homogeneous"), expression(paste("Total Memory Growth (", S[h], ")")), FALSE, 0.7, 240)
# Export
ggsave("selectivity-memory-homoEstRel.pdf", width = 10, height = 5, units = "cm", device = cairo_pdf)
heteroPerfMugPlot <- plotMug(mugHeteroPerf %>% filter(resources == "heterogeneous"), expression(paste("Total Memory Growth (", S[v], ")")), FALSE, 0.7, 240)
# Export
ggsave("selectivity-memory-hetero.pdf", width = 10, height = 5, units = "cm", device = cairo_pdf)
heteroPerfMugPlotEstRel <- plotMug(mugHeteroPerfEstRel %>% filter(resources == "heterogeneous"), expression(paste("Total Memory Growth (", S[v], ")")), FALSE, 0.7, 240)
# Export
