...
plotTheme <- function() {
  return(theme(legend.position = "none",#c(case_when(legendRight ~ 0.91, TRUE ~ 0.1), 0.9),
              plot.title = element_text(size = 12, margin = margin(0, 0, 0, 0, "cm")),
...
homoPerfRtPlot <- plotRt(rtHomoPerf %>% filter(resources == "homogeneous"), expression(paste("Run Time (", S[homo], ")")))
# Export
ggsave("performance-runtime-homo.pdf", width = 10, height = 5.4, units = "cm", device = cairo_pdf)
heteroPerfRtPlot <- plotRt(rtHeteroPerf %>% filter(resources == "heterogeneous"), expression(paste("Run Time (", S[hetero], ")")))
# Export
...
homoPerfMugPlot <- plotMug(mugHomoPerf %>% filter(resources == "homogeneous"), expression(paste("Total Memory Growth (", S[homo], ")")))
# Export
ggsave("performance-memory-homo.pdf", width = 10, height = 5.4, units = "cm", device = cairo_pdf)
heteroPerfMuglot <- plotMug(mugHeteroPerf %>% filter(resources == "heterogeneous"), expression(paste("Total Memory Growth (", S[hetero], ")")))
# Export
...
homoPerfCpuPlot <- plotCpu(cpuHomoPerf %>% filter(resources == "homogeneous"), expression(paste("Total CPU Time (", S[homo], ")")))
# Export
ggsave("performance-cpu-homo.pdf", width = 10, height = 5.4, units = "cm", device = cairo_pdf)
heteroPerfCpuPlot <- plotCpu(cpuHeteroPerf %>% filter(resources == "heterogeneous"), expression(paste("Total CPU Time (", S[hetero], ")")), FALSE)
# Export
...
homoPerfRtPlotEstRel <- plotRt(rtHomoPerfEstRel %>% filter(resources == "homogeneous"), expression(paste("Run Time (", S[homo], ")")), FALSE)
# Export
...
heteroPerfRtPlotEstRel <- plotRt(rtHeteroPerfEstRel %>% filter(resources == "heterogeneous"), expression(paste("Run Time (", S[hetero], ")")))
# Export
...
homoPerfCpuPlot <- plotCpu(cpuHomoPerf %>% filter(resources == "homogeneous"), expression(paste("Total CPU Time (", S[homo], ")")), FALSE)
# Export
ggsave("selectivity-cpu-homo.pdf", width = 10, height = 5, units = "cm", device = cairo_pdf)
homoPerfCpuPlotEstRel <- plotCpu(cpuHomoPerfEstRel %>% filter(resources == "homogeneous"), expression(paste("Total CPU Time (", S[homo], ")")))
# Export
...
heteroPerfCpuPlotEstRel <- plotCpu(cpuHeteroPerfEstRel %>% filter(resources == "heterogeneous"), expression(paste("Total CPU Time (", S[hetero], ")")))
# Export
...
homoPerfMugPlot <- plotMug(mugHomoPerf %>% filter(resources == "homogeneous"), expression(paste("Total Memory Growth (", S[homo], ")")), FALSE, 0.7)
# Export
ggsave("selectivity-memory-homo.pdf", width = 10, height = 5, units = "cm", device = cairo_pdf)
homoPerfMugPlotEstRel <- plotMug(mugHomoPerfEstRel %>% filter(resources == "homogeneous"), expression(paste("Total Memory Growth (", S[homo], ")")), FALSE, 0.7)
# Export
ggsave("selectivity-memory-homoEstRel.pdf", width = 10, height = 5, units = "cm", device = cairo_pdf)
heteroPerfMugPlot <- plotMug(mugHeteroPerf %>% filter(resources == "heterogeneous"), expression(paste("Total Memory Growth (", S[hetero], ")")), FALSE, 0.7)
# Export
ggsave("selectivity-memory-hetero.pdf", width = 10, height = 5, units = "cm", device = cairo_pdf)
heteroPerfMugPlotEstRel <- plotMug(mugHeteroPerfEstRel %>% filter(resources == "heterogeneous"), expression(paste("Total Memory Growth (", S[hetero], ")")), FALSE, 0.7)
# Export
...
statsPlot <- function(data, title, yvalue, sd, minY = 0, maxY = 1.1, legendRight = TRUE) {
  return(ggplot(data, aes(y=yvalue, x=costFunction, fill=placement)) +
          geom_bar(position="dodge", stat="identity") +
